...
public abstract class CompositeShape extends Shape {
	 ...
	 * @return List<Shape>
	 */
	public List<Shape> getShapesByType(ShapeType shape) {
		List<Shape> SpecificShapeList = new ArrayList<Shape>();
		for (Shape temporary : shapes) {
			if(temporary.getType().equals(shape)){
				SpecificShapeList.add(temporary);
			}
		}
		return SpecificShapeList;
	}
